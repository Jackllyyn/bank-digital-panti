@extends('layouts.app')
@section('title', 'Transaksi Penjualan/Pemakaian Barang')
...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        ...
        <a href="{{ route('admin.penjualan-pemakaian-barang.create') }}"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
        <form method="GET" action="{{ route('admin.penjualan-pemakaian-barang.index') }}" class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end" x-data="dateFilter()">
            <div>
            ...
            <div class="flex items-end gap-3">
                ...
                <a href="{{ route('admin.penjualan-pemakaian-barang.index') }}" class="text-gray-600 hover:text-gray-800 text-sm underline">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($keluars as $keluar)
                        ...
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $keluar->no_transaksi }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ \Carbon\Carbon::parse($keluar->tanggal)->format('d/m/Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">
                                {{ $keluar->barang->nama_barang ?? '-' }}
                                <span class="text-xs text-gray-400 block">{{ $keluar->barang->kode_barang ?? '' }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                {{ number_format($keluar->qty, 0, ',', '.') }} {{ $keluar->barang->satuan ?? 'Unit' }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium flex justify-center items-center gap-2">
                                <a href="{{ route('admin.persediaan-keluar.print', $keluar->id) }}" target="_blank"
                                ...
                                <a href="{{ route('admin.penjualan-pemakaian-barang.edit', $keluar->id) }}" 
                                ...
                                <form action="{{ route('admin.penjualan-pemakaian-barang.destroy', $keluar->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Apakah Anda yakin ingin menghapus transaksi ini? Stok akan dikembalikan.');">
                                    @csrf
                                    @method('DELETE')
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-gray-200">
            {{ $keluars->appends(request()->query())->links('vendor.pagination.tailwind') }}
        </div>
    </div>
</div>
@endsection
