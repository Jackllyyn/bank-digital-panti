        ...
        <form action="{{ route('admin.penjualan-pemakaian-barang.store') }}" method="POST" 
            ...
            <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <input type="date" name="tanggal" value="{{ old('tanggal', $keluar->tanggal->format('Y-m-d')) }}" required
                ...
                <div class="border-t border-gray-200 pt-4">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-4">
                        ...
                        <div class="md:col-span-5">
                            ...
                            <select name="barang_id" @change="updateInfo($event)" required class="block w-full border border-gray-300 rounded-md shadow-sm text-sm focus:ring-green-500 focus:border-green-500 p-2">
                                ...
                                @foreach($barangs as $b)
                                    <option value="{{ $b->id }}" data-price="{{ $b->harga_beli_rata2 }}" {{ old('barang_id', $keluar->barang_id) == $b->id ? 'selected' : '' }}>
                        ...
                        <div class="md:col-span-5" x-show="jenis === 'penjualan'">
                            ...
                            <input type="number" name="harga_jual" value="{{ old('harga_jual', $harga_jual) }}" min="0" class="block w-full border border-gray-300 rounded-md shadow-sm text-sm focus:ring-green-500 focus:border-green-500 p-2">
                        </div>
                            ...
                            <div>
                                ...
                                <select name="kode_akun_kas" class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-green-500 focus:border-green-500 px-3 py-2">
                                    ...
                                    @foreach($akunKas as $ak)
                                        <option value="{{ $ak->kode_akun }}" {{ old('kode_akun_kas', $kode_akun_kas) == $ak->kode_akun ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="kode_akun_pendapatan" class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-green-500 focus:border-green-500 px-3 py-2">
                                    ...
                                    @foreach($akunPendapatan as $ap)
                                        <option value="{{ $ap->kode_akun }}" {{ old('kode_akun_pendapatan', $kode_akun_pendapatan) == $ap->kode_akun ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="kode_akun_beban" class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-green-500 focus:border-green-500 px-3 py-2">
                                    ...
                                    @foreach($akunBeban as $ab)
                                        <option value="{{ $ab->kode_akun }}" {{ old('kode_akun_beban', $kode_akun_beban) == $ab->kode_akun ? 'selected' : '' }}>
                            ...
                            <div>
                                ...
                                <select name="kode_akun_persediaan" class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-green-500 focus:border-green-500 px-3 py-2">
                                    ...
                                    @foreach($akunPersediaan as $ap)
                                        <option value="{{ $ap->kode_akun }}" {{ old('kode_akun_persediaan', $kode_akun_persediaan) == $ap->kode_akun ? 'selected' : '' }}>
                                            {{ $ap->kode_akun }} - {{ $ap->nama_akun }}
                                        </option>
                                    @endforeach
                                </select>
                ...
                <div>
                    ...
                    <textarea name="keterangan" rows="3" placeholder="Contoh: Penjualan sisa barang bekas / Pemakaian untuk kegiatan panti"
                              class="mt-1 block w-full border border-gray-300 rounded-lg shadow-sm focus:ring-green-500 focus:border-green-500 px-3 py-2">{{ old('keterangan', $keterangan_clean) }}</textarea>
                </div>
            </div>
