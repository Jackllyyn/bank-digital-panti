    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <div class="px-6 py-5 bg-green-600 text-white">
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h1 class="text-xl font-semibold">Kas Kecil</h1>
                    <p class="text-green-100 text-sm mt-1">Catat Kas Kecil panti</p>
                </div>
                <a href="{{ route('admin.kas-kecil.create') }}"
        ...
        <div class="p-6">
           ...
           <div class="mb-2 flex flex-wrap gap-3">
                <a href="{{ route('admin.kas-kecil.export.excel') . '?' . http_build_query(request()->query()) }}"
                        ...
                        @forelse($kas_kecils as $p)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-5 py-3 text-center text-sm space-x-2">
                                    <a href="{{ route('admin.kas-kecil.edit', $p->id) }}" class="text-blue-600 hover:text-blue-900">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('admin.kas-kecil.destroy', $p->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Hapus data?')">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <td class="px-5 py-3 text-center text-sm space-x-2">
                                    <a href="{{ route('admin.kas-kecil.print', $p->id) }}" 
                                       target="_blank" 
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-10 text-sm text-gray-500">
                                    Belum ada data transaksi kas kecil.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="mt-6 flex justify-center">
                {{ $kas_kecils->withQueryString()->links('vendor.pagination.tailwind') }}
            </div>
        </div>
    </div>
</div>
