...
<div class="max-w-4xl mx-auto py-6">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <div class="px-6 py-5 bg-green-600 text-white">
            <div class="flex items-center">
                <a href="{{ route('admin.kas-kecil.index') }}" class="mr-4 text-white hover:text-green-100">
                    <i class="fas fa-arrow-left text-xl"></i>
                </a>
                <div>
                    <h1 class="text-xl font-semibold">Edit Kas Kecil</h1>
                    <p class="text-green-100 text-sm mt-1">Perbarui data transaksi kas kecil</p>
                </div>
            </div>
        </div>
    ...
    {{-- Transaksi --}}
     <div class="p-6">
            <form action="{{ route('admin.kas-kecil.update', $kasKecil->id) }}" method="POST" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" name="tanggal" value="{{ old('tanggal', $kasKecil->tanggal->format('Y-m-d')) }}" required
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">
                            Sumber Dana (Kredit) <span class="text-gray-400 font-normal text-xs">- Kas Kecil Berkurang</span> *
                        </label>
                        <select name="nama_akunk" required
                            ...
                            @foreach($akuns as $akun)
                                @if (stripos($akun->nama_akun, 'Kas') !== false || stripos($akun->nama_akun, 'Bank') !== false)
                                    <option value="{{ $akun->kode_akun }}" {{ old('nama_akunk', $kasKecil->kode_akunk) == $akun->kode_akun ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="nama_akund" required
                            ...
                            @foreach($akuns as $akun)
                                <option value="{{ $akun->kode_akun }}" {{ old('nama_akund', $kasKecil->kode_akund) == $akun->kode_akun ? 'selected' : '' }}>
                                    {{ $akun->kode_akun }} - {{ $akun->nama_akun }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="jumlah" value="{{ old('jumlah', $kasKecil->jumlah) }}" required min="1" step="0.01"
                    ...
                    <div>
                        ...
                        <input type="text" name="no_bukti" value="{{ old('no_bukti', $kasKecil->no_bukti) }}"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 text-sm"
                    ...
                    <textarea name="keterangan" rows="4" required
                              ...
                              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 text-sm">{{ old('keterangan', $kasKecil->keterangan) }}</textarea>
                ...
                <div class="flex justify-end gap-4 pt-4 border-t border-gray-200">
                    <a href="{{ route('admin.kas-kecil.index') }}"
