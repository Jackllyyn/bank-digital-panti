@extends('layouts.app')
@section('title', 'Input Kas Kecil Baru')
...
<div class="max-w-4xl mx-auto py-6">
    <div class="bg-white rounded-xl shadow-sm border border-gray-200">
        ...
        <div class="px-6 py-5 bg-green-600 text-white">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-xl font-semibold">Input Kas Kecil</h1>
                    <p class="text-green-100 text-sm mt-1">Catat pengeluaran kas kecil</p>
                </div>
            </div>
        </div>
    ...
    {{-- Transaksi --}}
     <div class="p-6">
            <form action="{{ route('admin.kas-kecil.index') }}" method="POST" class="space-y-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="nama_akunk" required
                            ...
                            @foreach($sumberDana as $akun)
                                <option value="{{ $akun->kode_akun }}" {{ old('nama_akunk') == $akun->kode_akun ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="nama_akund" required
                            ...
                            @foreach($akuns as $akun)
                                <option value="{{ $akun->kode_akun }}" {{ old('nama_akund') == $akun->kode_akun ? 'selected' : '' }}>
                                    {{ $akun->kode_akun }} - {{ $akun->nama_akun }}
                                </option>
                            @endforeach
                        </select>
