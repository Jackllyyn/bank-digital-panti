@extends('layouts.app')
@section('title', 'Daftar Donasi Barang')
...
<div class="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
        ...
        <a href="{{ route('admin.donasi-barang.create') }}"
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-6">
        <form method="GET" action="{{ route('admin.donasi-barang.index') }}" class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-end" x-data="dateFilter()">
            <div>
            ...
            <div class="flex items-end gap-3">
                ...
                <button type="submit" formaction="{{ route('admin.donasi-barang.export.pdf') }}" formtarget="_blank" class="px-6 py-2.5 bg-red-600 text-white text-sm font-medium rounded-lg hover:bg-red-700 transition flex items-center gap-2">
                    <i class="fas fa-file-pdf"></i> PDF
                </button>
                <a href="{{ route('admin.donasi-barang.index') }}" class="text-gray-600 hover:text-gray-800 text-sm underline">Reset</a>
            </div>
        </form>
    </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($donasis as $donasi)
                        <tr class="hover:bg-gray-50 transition donasi-row cursor-pointer" data-id="{{ $donasi->id }}">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{{ $donasi->no_transaksi }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $donasi->tanggal->format('d/m/Y') }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $donasi->donatur?->nama ?? 'Umum' }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $donasi->akun?->nama_akun ?? $donasi->kode_akun }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                {{ number_format($donasi->details_sum_qty ?? 0, 0, ',', '.') }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-bold text-green-700">
                                Rp {{ number_format($donasi->details_sum_total_nilai ?? 0, 0, ',', '.') }}
                            </td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium flex justify-center items-center gap-2">
                                <a href="{{ route('admin.donasi-barang.show', $donasi->id) }}" 
                                ...
                                <a href="{{ route('admin.donasi-barang.edit', $donasi->id) }}" 
                                ...
                                <form action="{{ route('admin.donasi-barang.destroy', $donasi->id) }}" method="POST" class="inline-block detail-link" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data donasi ini? Stok dan jurnal akan dibatalkan.');">
                                    @csrf
                                    @method('DELETE')
