...
<div x-data="{ showForceDeleteAllConfirm: false }" class="max-w-full mx-auto px-4 py-8">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
        <h1 class="text-2xl font-semibold text-gray-800">Trash Donatur</h1>
    ...
    <div x-show="showForceDeleteAllConfirm" x-transition class="bg-red-50 border border-red-300 rounded-xl p-6 mb-8">
        ...
        <p class="text-gray-700 mb-6">
            Tindakan ini <strong>tidak dapat dibatalkan</strong> dan akan menghapus <strong>semua data donatur</strong> yang ada di dalam trash secara permanen.
        </p>

        <form action="{{ route('admin.donatur.force-delete-all') }}" method="POST" class="flex flex-wrap items-center gap-4">
            @csrf
            <label class="flex items-center text-red-700 font-medium">
                ...
                Saya mengerti dan ingin menghapus SEMUA data di trash
            </label>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                @forelse($donaturs as $donatur)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $donatur->kota ?? '—' }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-center text-sm">
                            <div class="flex items-center justify-center gap-4">
                                ...
                                <form action="{{ route('admin.donatur.forceDelete', $donatur->kode_donatur) }}" method="POST" class="inline" onsubmit="return confirm('Anda yakin ingin menghapus data ini secara PERMANEN? Tindakan ini tidak dapat dibatalkan.')">
                                    @csrf
                                    @method('DELETE')
                ...
                @empty
                    <tr>
                        <td colspan="4" class="text-center py-16 text-gray-500">
                            ...
                            <p class="mt-2">Tidak ada data donatur yang dihapus.</p>
                        </td>
                    </tr>
                @endforelse
                </tbody>
